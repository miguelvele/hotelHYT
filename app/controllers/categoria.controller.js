...
const categorias = async (req, res) => {
    ...
    try {
        ...
        let ruta = process.env.API + 'categoria';
        let option = {
            method: "GET",
        }
        let datos = {};
...
const guardarcate = async (req, res) => {
    const nuevaCate = {
...
const borrarcate = async (req, res) => {
    ...
    try {
        ...
        const url = process.env.API + 'categoria' + `/${id}`;
        const option = {
            method: "DELETE"
        };
        const result = await fetch(url, option)
            ...
            .then((data) => {
                if (data[0].affectedRows == 1) {
                    res.redirect(`/v1/categorias?success=true&id=${id}`);
                } else {
                    console.log("No se pudo borrar mi bro");
                    res.redirect(`/v1/categorias?success=false&id=${id}`);
                }
            });
    } catch (error) {
...
const editcatego = async (req, res) => {
...
const guardarcategoria = async (req, res) => {
    ...
    let ruta = process.env.API + 'categoria/' + req.body.id;
...
export const categoriaController = {
    categorias, guardarcate, borrarcate, salir, generarpdf, editcatego, guardarcategoria
};