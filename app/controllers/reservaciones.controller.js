...
const editreserva = async (req, res) => {
  ...
  // Recupera los usuarios y los métodos de pago de la API
 
  try {
    const usuariosRes = await fetch(process.env.API + 'usuarios');
    const usuarios = await usuariosRes.json();

    const metodopagoRes = await fetch(process.env.API + 'metodopago');
    const metodosPago = await metodopagoRes.json();
