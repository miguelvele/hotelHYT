...
const estadosh = async (req, res) => {


    try {
        // process.env.API + 'pro' + `/${id}`;

        let ruta = process.env.API + 'estadohabitacion';
        let option = {
            method: "GET",
        }
        let datos = {};
...
const guardarestado = async (req, res) => {
    const nuevoUsuario = {
    ...
    axios.post(process.env.API + 'estadohabitacion', nuevoUsuario)
        .then(response => {
            console.log(response.data);
            // Aquí puedes realizar alguna acción adicional o mostrar un mensaje de éxito.
            res.redirect("/v1/estadosh");
        })
        .catch(error => {
            console.error(error);
...
const borrarestado = async (req, res) => {
    ...
    try {
        ...
        const url = process.env.API + 'estadohabitacion' + `/${id}`;
        const option = {
            method: "DELETE"
        };
        const result = await fetch(url, option)
            ...
            .then((data) => {
                if (data[0].affectedRows == 1) {
                    res.redirect(`/v1/estadosh?success=true&id=${id}`);
                } else {
                    console.log("No se pudo borrar mi bro");
                    res.redirect(`/v1/estadosh?success=false&id=${id}`);
                }
            });
    } catch (error) {
...
const guardarestados = async (req, res) => {
    ...
    let ruta = process.env.API + 'estadohabitacion/' + req.body.id;
...
export const estadoController = {
    estadosh, guardarestado, borrarestado, salir, actualizar, generarpdf, editestado, guardarestados
};