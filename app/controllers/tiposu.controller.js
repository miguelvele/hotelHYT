...
const tiposu = async (req, res) => {


    try {
        // process.env.API + 'pro' + `/${id}`;

        let ruta = process.env.API + 'tiposusuario';
        let option = {
            method: "GET",
        }
        let datos = {};
...
const guardartiposu = async (req, res) => {
    const nuevoUsuario = {
...
const borrartiposu = async (req, res) => {
    ...
    try {
        ...
        const url = process.env.API + 'tiposusuario' + `/${id}`;
        const option = {
            method: "DELETE"
        };
        const result = await fetch(url, option)
            ...
            .then((data) => {
                if (data[0].affectedRows == 1) {
                    res.redirect(`/v1/tiposu?success=true&id=${id}`);
                } else {
                    console.log("No se pudo borrar mi bro");
                    res.redirect(`/v1/tiposu?success=false&id=${id}`);
                }
            });
    } catch (error) {
...
const edittiposu = async (req, res) => {
    const id = req.query.id;
...
const guardartiposusuario = async (req, res) => {
    ...
    let ruta = process.env.API + 'tiposusuario/' + req.body.id;
...
export const tiposuController = {
    tiposu, guardartiposu, borrartiposu, salir, actualizar, generarpdf, edittiposu, guardartiposusuario
};